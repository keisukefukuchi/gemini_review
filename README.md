...
プルリクエストがマージされると、以下の処理が自動で実行されます：

- **ブランチの自動削除**: `feature/*`、`fix/*`、`hotfix/*`、`bugfix/*` で始まるブランチが自動で削除されます
- **Issueの自動クローズ**: プルリクエストの本文やタイトルに `closes #123`、`fixes #456`、`#789` などの形式でissue番号が含まれている場合、該当するissueが自動でクローズされます

#### Issueクローズの記述方法

プルリクエストの本文やタイトルに以下のいずれかの形式でissue番号を記載してください：
...
複数のissueをクローズする場合は、複数行に分けて記載できます。
